...
if (!global.server) {
    let possible = global.servers.filter(r => r.visible && r.prefer);
...
function getColor(colorNumber = '#FF0000') {
    ...
    switch (colorNumber) {
        ...
        default: // It's color string, just return
            return colorNumber
        //return '#FF0000';
    }
}
...
function getZoneColor(cell, real) {
    switch (cell) {
        ...
        case (cell.match(/(dom[0-9])|(san[0-9])/) || {}).input:
...
window.onload = function () {
    ...
    */
    ...
    util.retrieveFromLocalStorage('optFont');
    if (document.getElementById('optFont').checked) {
      document.documentElement.style.setProperty('--font', 'Ubuntu');
    };
    document.getElementById('optFont').onclick = function (){
      let newFont = document.getElementById('optFont').checked ? "Ubuntu" : "Quicksand";
      document.documentElement.style.setProperty('--font', document.getElementById('optFont').checked ? "Ubuntu" : "Quicksand");
    };
...
async function startGame() {
    ...
    util.submitToLocalStorage('optFont');
    if (document.getElementById('optFont').checked) global.font = "Ubuntu";
...
var measureText = function () {
    ...
    return function (text, fontSize) {
        ...
        var w, h;
        div.style.font = 'bold ' + fontSize + 'px ' + global.font;
        div.style.padding = '0';
        div.style.margin = '0';
        ...
        return {
            draw: function draw(text, x, y, size, fill) {
                ...
                if (floppies.some(function (f) {
                    ...
                    tctx.font = 'bold ' + size + 'px ' + global.font;
                    tctx.textAlign = align; tctx.textBaseline = 'middle';
                    tctx.strokeStyle = color.black;
                    tctx.fillStyle = fill;
                    tctx.lineCap = (config.graphical.pointy) ? 'miter' : 'round'; ////
                    tctx.lineJoin = 'round';
                ...
                ctx.drawImage(tctx.canvas, Math.floor(x - xx), Math.floor(y - yy * (1.05 + !center * 0.45)));
                ctx.restore();
            },
...
var drawEntity = function () {
    ...
    function drawPoly(context, centerX, centerY, radius, sides, angle = 0, fill = true) {
        ...
        if (!sides) {
        ...
        } else if (sides > 0 && sides < 10000) {
            angle += sides % 2 ? 0 : Math.PI / sides;

            // Polygon
            for (var i = 0; i < sides; i++) {
                var theta = i / sides * 2 * Math.PI;
                var x = centerX + radius * Math.cos(theta + angle);
                var y = centerY + radius * Math.sin(theta + angle);
                context.lineTo(x, y);
            }
        } else if (sides >= 10000) {
            // Image
            const imgs = {
                ...
                10001: 'img/big_brother.jpg',
                10002: 'img/asteroid.png',
                10003: 'img/as.png',
                10004: 'img/amogus.jpg',
                10005: 'img/amogus2.png'
            };
    ...
    return (x, y, instance, ratio, alpha = 1, scale = 1, rot = 0, turretsObeyRot = false, assignedContext = false, turretInfo = false, render = instance.render) => {
        try {
            ...
            context.lineCap = (config.graphical.pointy) ? 'miter' : 'round';
...
var gameDraw = function () {
    ...
    return function (ratio) {
        ...
        do {
            ...
            if (!basic) break;
            global.realScrollX = lerp(global.realScrollX, global.scrollX, 0.1);
            let tiles = [],
                branches = [],
                measureSize = (x, y, colorIndex, {
                    ...
                    switch (tier) {
                        ...
                        case 2:
                            ...
                            if (upgrades.length) branches.push([{ x, y }, { x, y: y + 1 + upgrades.length }]);
                            return {
                                width: 1,
            ...
            for (let [start, end] of branches) {
                let sx =
                    ...
                    (start.x - full.width * global.realScrollX) * tileSize +
                    1 +
                    0.5 * size,
                    sy =
                    ...
                    ex =
                        global.screenWidth / 2 +
                        (end.x - full.width * global.realScrollX) * tileSize +
                        1 +
                        0.5 * size,
                    ey =
                        global.screenHeight / 2 +
            ...
            */
            ...
            for (let {
                ...
                drawGuiRoundRect(ax, ay, size, size, 10, true);
                ctx.globalAlpha = 0.15;
                ctx.fillStyle = getColor(0);
                drawGuiRect(ax, ay, size, size * 0.6, 10, true, false);
                ctx.fillStyle = color.black;
                drawGuiRect(ax, ay + size * 0.6, size, size * 0.4, 10, true, false);
                ctx.globalAlpha = 1;
                let angle = Date.now() / 2500,
...

        {
            ...
            // Text
            if (global.showDebug) {
                text.debug[6].draw('Acorent.io', _x29 + _len7, _y5 - 50 - 6 * 14, 14, mixColors(color.yellow, color.green), 'right');
                ...
                text.debug[3].draw('Acorent.io', _x29 + _len7, _y5 - 10 - 3 * 14, 14, mixColors(color.yellow, color.green), 'right');
...
var gameDrawBeforeStart = function () {
    ...
    return function () {
        ...
        global.animations.connecting = lerp(global.animations.connecting, 0, 0.1);
        ctx.translate(0, global.animations.connecting * global.screenHeight);
...
async function animloop() {
    global.scaleGuiRatio = Math.max(global.screenWidth, (global.screenHeight * 16) / 9) / (global.screenWidth <= 1280 ? 1280 : global.screenWidth >= 1920 ? 1920 : global.screenWidth);
    ...
    /*
      ctx.scale(global.scaleGuiRatio, global.scaleGuiRatio);
      global.screenWidth /= global.scaleGuiRatio;
      global.screenHeight /= global.scaleGuiRatio;
    */
    ...
    /*
      ctx.scale(1 / global.scaleGuiRatio, 1 / global.scaleGuiRatio);
      global.screenWidth *= global.scaleGuiRatio;
      global.screenHeight *= global.scaleGuiRatio;
      */
}