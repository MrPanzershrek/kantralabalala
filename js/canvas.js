  ...
  keyboardDown(event) {
    switch (event.keyCode) {
      ...
      case global.KEY_TESTBED:
        ...
        case global.KEY_KILL_ENTITY:
        this.parent.socket.talk('V'); break;
    }
    if (!event.repeat) {
      switch (event.keyCode) {
        ...
        case global.KEY_UPGRADE_MAX:
          this.statMaxing = true; break;
        case global.KEY_CLASS_TREE:
          global.showTree = !global.showTree;
          if (global.showTree === true) {
            for (let message of ["Press Y again to close it.", "Use the left and right arrow keys to scroll.", "Use the up and down arrow keys to jump from start to end.", "Class tree controls:"]) {
        ...
        case global.KEY_SUICIDE:
          this.parent.socket.talk('K'); break;
        case global.KEY_TP:
          this.parent.socket.talk('T'); break;
        case global.KEY_PASSIVE:
          this.parent.socket.talk('P'); break;
        case global.KEY_GOD:
          this.parent.socket.talk('G'); break;
        case global.KEY_DEBUG:
          global.showDebug = true; break;
        case global.KEY_SCREENSHOT:
          const url = this.parent.cv.toDataURL("image/png"); // .replace("image/png", "image/octet-stream")
          const a = document.createElement('a');
          a.style.display = 'none';
          a.href = url;
          a.download = 'diep3.png';
          ...
          break;
        case global.KEY_VIDEO:
          if (window.MediaRecorder && this.parent.cv.captureStream) {
            if (!this.mediaRecorder) {
              let recorded = [];
              this.mediaRecorder = new MediaRecorder(this.parent.cv.captureStream(60));

              this.mediaRecorder.ondataavailable = (event) => recorded.push(event.data);

              this.mediaRecorder.onstart = () => {
                recorded = [];
                global.messages.push({
                  text: 'Video recorder started.',
                  status: 2,
                  alpha: 0,
                  time: Date.now(),
                  colorIndex: 9
              ...
              this.mediaRecorder.onstop = () => {
                global.messages.push({
                  text: 'Video recorder stopped. Saving file...',
                  status: 2,
                  alpha: 0,
                  time: Date.now(),
                  colorIndex: 9
          ...
          } else global.messages.push({
            text: 'Video recorder is not supported in this browser.',
            status: 2,
            alpha: 0,
            time: Date.now(),
            colorIndex: 9
